...
class parse
{
    ...
    public function bbcodes($content)
    {
        ...
        while (preg_match("#\[code\](.+?)\[/code\]#isu", $content, $res)) {
            $content = str_replace($res[0], '<pre><code>'.str_replace(['[', '<'], ['&#91;', '&lt;'], $res[1]).'</code></pre>', $content);
        }
		...
		// Process h
        while (preg_match("#\[h([1-6]{1})\](.+?)\[/h\\1\]#is", $content, $null))
           $content    =    preg_replace("#\[h([1-6]{1})\](.+?)\[/h\\1\]#is", "<h\\1>\\2</h\\1>", $content);
		// Process bgcolor
		...
		// Process font size
       while (preg_match("#\[size=([0-9]+)\](.+?)\[/size\]#is", $content, $null))
           $content    =    preg_replace("#\[size=([0-9]+)\](.+?)\[/size\]#is", '<font size="$1">$2</font>', $content);

		// Process font face
        while (preg_match("#\[font=([a-zA-Z\s]+)\](.*?)\[/font\]#is", $content, $null))
           $content    =    preg_replace("#\[font=([a-zA-Z\s]+)\](.*?)\[/font\]#is", '<font face="$1">$2</font>', $content);  
        ...
        if (preg_match_all("#\[img(\=| *)(.*?)\](.*?)\[\/img\]#isu", $content, $pcatch, PREG_SET_ORDER)) {
            ...
            foreach ($pcatch as $catch) {
				...
				$style = " ";
				if( intval( $config['images_url_w'] ) ) {
					if ($config['http_home_url'] != $urlREF ) {
						$style .= " style=\"max-width:".intval($config['images_url_w'])."px;\" data-maxwidth=\"".intval($config['images_url_w'])."\"";
					}
				}
                // Fill an output replacing array
                array_push($rdest, '<a href="'.$urlREF.'" rel="nofollow" onclick="return hs.expand (this)" class="highslide"><img '.(implode(' ', $outkeys)).' ' . $style . ' /></a>');
            }

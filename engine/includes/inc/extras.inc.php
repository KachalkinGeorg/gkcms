...
function plugins_load_version_file($filename)
{
    ...
    $config_params = ['id', 'name', 'version', 'acts', 'file', 'config', 'install', 'deinstall', 'management', 'type', 'description', 'author', 'author_uri', 'permanent', 'library', 'actions', 'minenginebuild', 'icon', 'information'];
...
function locatePluginTemplates($tname, $plugin, $localsource = 0, $skin = '', $block = '')
{
    ...
    foreach ($tname as $fn) {
        ...
        } elseif (is_readable(extras_dir . '/'.$plugin.'/tpl/admin/'.$fnc)) {
            $tpath[$fn] = extras_dir . '/'.$plugin.'/tpl/admin/';
            $tpath['url:' . $fn] = admin_url . '/plugins/'.$plugin.'/tpl/admin/';
        }
    }
...
function _MASTER_defaultRUN($pluginName, $handlerName, $params, &$skip, $handlerParams)
{
    ...
    if (is_array($pcall) && function_exists($pcall['func'])) {
		...
		$SYSTEM_FLAGS['meta']['description'] = 'Плагин '.$pluginName.' либо недоступен, или введен неверный адрес.';

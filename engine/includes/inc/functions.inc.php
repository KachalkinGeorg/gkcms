...
function on_of_line($id)
{
	...
	if ($config['on_of_line']) {
		...
		$row = $mysql->record("select * from " . uprefix . "_users where id = " . intval($id));
		$last = $row['last'] + 300;
...
function print_show_profile($id, $user, $text)
{
    ...
    if ($config['use_avatars']) {
        ...
        if ($config['avatars_gravatar']) {
            $avatars = 'http://www.gravatar.com/avatar/'.md5(mb_strtolower($row['mail'])).'.jpg?s='.$config['avatar_wh'].'&d='.urlencode($noAvatarURL);
            } else {
               $avatars = avatars_url.'/noavatar.gif';
            }
        }
    }
...
/* function print_modal_dialog($id, $title, $text, $action)
{
    ...
    $tpl->template('modal', tpl_actions.'config');
    $tvars['vars'] = ['id' => $id, 'title' => $title, 'text' => $text, 'action' => $action, 'php_self' => $PHP_SELF];
    $tpl->vars('modal', $tvars);
    print $tpl->show('modal');
} */
...
function breadcrumb($title, $subtitle) {
	...
	$admin .= ' <div class="content-wrapper">
	<div class="row mb-3">
	  <div class="col-sm-9 d-none d-md-block" style="margin: 20px;">
...
// Уведомления о завершении изменения конфигурации
function print_msg($type, $title, $text, $back = false) {
	global $PHP_SELF, $main_admin, $lang, $breadcrumb, $plugin;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-comment-o btn-position"></i><span class="text-semibold">'.$title.'</span>', $titl );
	...
	$admin = '
<div class="alert ' . $type . ' alert-styled-left alert-arrow-left alert-component message_box" style="padding:0px">
  <h4>' . $title . '</h4>
...
function msg($params, $mode = 0, $disp = -1)
{
    ...
    $tmvars = [
        'vars' => [
            'text' 		=> trim(db_squote($text), "'"),
            'info' 		=> trim(db_squote($info), "'"),
			'title' 	=> $title,
...
//			1 - print
//			2 - return as result
function msgSticker($msg, $title, $info, $type = '', $disp = -1)
{
    global $notify, $twig;
    ...
    if (is_array($info)) {
        ...
        $lines[] = htmlspecialchars($info, ENT_COMPAT | ENT_HTML401, 'UTF-8');
    }
	
	if ($title) {
		$titles = trim(db_squote('<b>'.$title.'</b><br />'), "'");
	} else {
		$titles = '';
	}
	...
	if ($info) {
		$info = trim(db_squote('<br /><i class="fa fa-exclamation"></i> '.$info.''), "'");
	} else {
		$info = '';
	}
...
global $tpl, $lang;
	...
	$tvars['vars'] = [
		...
		'error' => isset($param['error']) ? (str_replace('%error%', $param['error'], $lang['param_error'])) : false,
		'input' => '',
		'value' => $value,
		'values' => $values,
	];
   ...
   if ($param['id']) {
      $tvars['vars']['id'] = isset($param['id']) ? secure_html($param['id']) : false;
   }
   
	if ($param['type'] == 'text') {
		$tvars['vars']['input'] = '<textarea name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . ' class="form-control">' . (isset($param['value']) ? $param['value'] : '') . '</textarea>';
	} elseif ($param['type'] == 'input') {
		$tvars['vars']['input'] = '<input type="text" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . ' value="' . $value . '" class="form-control" />';
		...
		$tvars['vars']['input'] = '<input type="hidden" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . '" value="' . $value . '" class="form-control" />';
	} elseif ($param['type'] == 'select') {
		$tvars['vars']['input'] = MakeDropDown($param['values'], $name, $value, ($id ? 'id="'.$id.'" ': '' ));
	} elseif ($param['type'] == 'button') {
		$tvars['vars']['input'] = '<input type="button" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . ' value="' . $value . '" class="btn btn-default" />';
	} elseif ($param['type'] == 'email') {
		$tvars['vars']['input'] = '<input type="email" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . '" value="' . $value . '" class="form-control" />';
	} elseif ($param['type'] == 'password') {
		$tvars['vars']['input'] = '<input type="password" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . '" value="' . $value . '" class="form-control" />';
	} elseif ($param['type'] == 'number') {
		$tvars['vars']['input'] = '<input type="number" name="' . $name . '" '. ($id ? 'id="'.$id.'" ': '' ). ' ' . $html_flags . '" value="' . $value . '" class="form-control" />';
	} elseif ($param['type'] == 'file') {
		$tvars['vars']['input'] = '<div class="btn btn-default btn-secondary btn-fileinput form-control"><span><i class="fa fa-plus"></i> Select file ...</span>
...
{
    global $lang, $catz, $config;

    $output = "<select name=\"orderby\">\n";
    if ($showDefault) {
		$order = $config['default_newsorder'] ? $config['default_newsorder'] : 'id desc';
        $output .= '<option value="'.$order.'">'.$lang['order_default'];
    }
    foreach (['id desc', 'id asc', 'postdate desc', 'postdate asc', 'editdate desc', 'editdate asc', 'title desc', 'title asc', 'rating desc', 'rating asc'] as $v) {
        $vx = str_replace(' ', '_', $v);
...
function newsFillVariables($row, $fullMode, $page = 0, $disablePagination = 0, $regenShortNews = [])
{
    ...
    if ($scrin) {
       ...
       $tvars['regx']["'\[noscrins\].*?\[/noscrins\]'si"] = '';
    } else {
       $tvars['vars']['[noscrins]'] = '';
       $tvars['vars']['[/noscrins]'] = '';

       $tvars['regx']["'\[scrins\].*?\[/scrins\]'si"] = '';
    }
    ...
    if ($fixed) {
       ...
       $tvars['regx']["'\[nofixed\].*?\[/nofixed\]'si"] = '';
    } else {
       $tvars['vars']['[nofixed]'] = '';
       $tvars['vars']['[/nofixed]'] = '';

       $tvars['regx']["'\[fixed\].*?\[/fixed\]'si"] = '';
    }
...
function coreUserMenu()
{
    ...
    if (!is_array($userROW)) {
		...
		$tVars['profile_show'] = print_show_profile($userROW['id'], 'Информация о профиле', 'ВНИМАНИЕ!<br>Вы не авторизованны!');
		$tVars['line'] = on_of_line($userROW['id']);;
    } else {
        // User is logged in
        ...
        $tVars['home_url'] = home;
		$tVars['profile_show'] = print_show_profile($userROW['id'], $userROW['name'], '');
		$tVars['line'] = on_of_line($userROW['id']);;

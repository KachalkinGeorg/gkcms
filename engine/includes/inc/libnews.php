...
function news_showone($newsID, $alt_name, $callingParams = [])
{
    ...
    $SYSTEM_FLAGS['meta']['robots'] = ($row['robots'] != '1') ? '' : 'noindex,nofollow';
	...
	$access = 0;
	if (!is_array($userROW)) $access = $row['acces'];
	else if (in_array($userROW['status'],$acces)) $access = $row['acces'];
...
function news_showlist($filterConditions = [], $paginationParams = [], $callingParams = [])
{
    ...
    if (!in_array($orderBy, ['id desc', 'id asc', 'postdate desc', 'postdate asc', 'editdate desc', 'editdate asc', 'title desc', 'title asc'])) {
        $orderBy = 'id desc';
    }
    ...
    switch ((isset($callingParams['fixed']) && $callingParams['fixed']) ? $callingParams['fixed'] : '') {
        default:
            $orderBy = 'fixed desc, '.$orderBy;
            break;
    }

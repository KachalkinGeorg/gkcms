    ...
    if (is_array($uRow = $mysql->record('select * from '.prefix.'_users where id='.db_squote($userid)))) {
        ...
        if ($uRow['activation']) {
            if ($uRow['activation'] == $code) {
                ...
                msg(['type' => 'info', 'text' => $lang['msgo_activated'], 'info' => sprintf($lang['msgi_activated'], admin_url)]);
                $SYSTEM_FLAGS['module.usermenu']['redirect'] = $config['home_url'].'/';
            } else {
                // Incorrect activation code
                msg(['type' => 'info', 'text' => $lang['msge_activate'], 'info' => sprintf($lang['msgi_activate'], admin_url)]);
            }
        } else {
            // Already activated
            echo 'ALREADY';
            msg(['type' => 'info', 'text' => $lang['msge_activation'], 'info' => sprintf($lang['msgi_activation'], admin_url)]);
        }
    } else {
        // User not found
        error404();
...
{
    global $template, $PHP_SELF, $config, $PFILTERS, $mysql, $parse, $twig, $twigLoader, $lang;

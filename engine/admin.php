...
$permissions = [
    'default'       => checkPermission(['plugin' => '#admin', 'item' => 'default'], null, 'details'),
	...
	'syslog'  		=> checkPermission(['plugin' => '#admin', 'item' => 'syslog'], null, 'details'),
	'loadlog'  		=> checkPermission(['plugin' => '#admin', 'item' => 'loadlog'], null, 'details'),
	'profillog'  	=> checkPermission(['plugin' => '#admin', 'item' => 'profillog'], null, 'details'),
...
if (isset($permissions[$mod]) && $permissions[$mod]) {
	...
	if ($userROW['status'] <= $level) {
		...
		msg(['type' => 'error', 'title' => $lang['notifyWindowInfo'], 'text' => $lang['msge_mod']]);
    }
} else {
    $notify = msg(['type' => 'error', 'text' => $lang['msge_mod']]);
	msg(['type' => 'error', 'title' => $lang['notifyWindowInfo'], 'text' => $lang['msge_mod']]);
	print_msg( 'warning', $lang['msge_mod'], $lang['msge_mod_desc'], ''.$config['admin_url'].'/admin.php' );
}
...
$tVars = [
    ...
    'h_active_options'    => (in_array($mod, ['categories', 'static', 'news', 'images', 'files'])) ? ' class="active"' : '',
    'h_active_system'     => (in_array($mod, ['options', 'configuration', 'dbo', 'rewrite', 'cron', 'newsletter', 'replace', 'statistics', 'default'])) ? ' class="active"' : '',
    ...
    'h_active_extras'     => (in_array($mod, ['extras', 'extra-config'])) ? ' class="active"' : '',
	'h_active_logs'       => (in_array($mod, ['syslog', 'loadlog', 'profillog'])) ? ' class="active"' : '',
    'h_active_pm'         => ($mod == 'pm') ? ' class="active"' : '',
	'h_active_help'       => (in_array($mod, ['docs'])) ? ' class="active"' : '',
    'year'                => date('Y'),
    ...
    'perm'                => [
        ...
        'options'       => checkPermission(['plugin' => '#admin', 'item' => 'options'], null, 'view'),
        'categories'    => checkPermission(['plugin' => '#admin', 'item' => 'categories'], null, 'view'),
    ...
    $xt = $twig->loadTemplate(dirname(tpl_actions).'/header.tpl');
    echo $xt->render($tVars);
}

...
class auth_basic extends CoreAuthPlugin
{
	...
	function get_reg_params()
	{
		...
		array_push($params, array('name' => 'alt_name', 'id' => 'reg_name', 'title' => $lang['auth_name'], 'descr' => $lang['auth_name_descr'], 'type' => 'input'));
		array_push($params, array('name' => 'gender', 'id' => 'reg_gender', 'title' => $lang['auth_gender'], 'descr' => $lang['auth_gender_descr'], 'type' => 'select', 'values' => array('0' => $lang['auth_gender_0'], '1' => $lang['auth_gender_1'], '2' => $lang['auth_gender_2'])));
		if (pluginGetVariable('auth_basic', 'group')){
			array_push($params, array('name' => 'group', 'id' => 'reg_group', 'title' => $lang['auth_group'], 'descr' => $lang['auth_group_descr'], 'type' => 'select', 'values' => $groupOptions ));
		}
		return $params;
	}
	...
	function register(&$params, $values, &$msg)
	{
		...
		if (pluginGetVariable('auth_basic', 'group')){
			$regGroup = secure_html(trim(db_squote($values['group'])));
		}else{
		...
		$alt_name = secure_html(trim(db_squote($values['alt_name'])));
		$gender = secure_html(trim(db_squote($values['gender'])));
		...
		switch ($config['register_type']) {
			...
			case 0:
				...
				$mysql->query("INSERT INTO " . uprefix . "_users (name, alt_name, pass, mail, status, reg, gender, last) VALUES (" . db_squote($values['login']) . ", " . $alt_name . ", " . db_squote(EncodePassword($newpassword)) . ", " . db_squote($values['email']) . ", " . $regGroup . ", '" . $add_time . "', " . $gender . ", '')");
			...
			case 2:
				...
				// Insert into DB
				$mysql->query("INSERT INTO " . uprefix . "_users (name, alt_name, pass, mail, status, reg, gender, last, activation) VALUES (" . db_squote($values['login']) . ", " . $alt_name . ", " . db_squote(EncodePassword($newpassword)) . ", " . db_squote($values['email']) . ", " . $regGroup . ", '" . $add_time . "', " . $gender . ", '', " . db_squote($actcode) . ")");
				$userid = $mysql->result('select LAST_INSERT_ID()');
			...
			case 3:
				$mysql->query("INSERT INTO " . uprefix . "_users (name, alt_name, pass, mail, status, reg, gender, last) VALUES (" . db_squote($values['login']) . ", " . $alt_name . ", " . db_squote(EncodePassword($values['password'])) . ", " . db_squote($values['email']) . ", " . $regGroup . ", '" . $add_time . "', " . $gender . ", '')");
				$userid = $mysql->result('select LAST_INSERT_ID()');
				$tvars['vars'] = array(
					'login'    => $values['login'],
					'home'     => home,
			...
			case 4:
				$actcode = MakeRandomPassword();
				$mysql->query("INSERT INTO " . uprefix . "_users (name, alt_name, pass, mail, status, reg, gender, last, activation) VALUES (" . db_squote($values['login']) . ", " . $alt_name . ", " . db_squote(EncodePassword($values['password'])) . ", " . db_squote($values['email']) . ", " . $regGroup . ", '" . $add_time . "', " . $gender . ", '', " . db_squote($actcode) . ")");
				$userid = $mysql->result('select LAST_INSERT_ID()');
				$link = generatePluginLink('core', 'activation', array('userid' => $userid, 'code' => $actcode), array(), false, true);
				$tvars['vars'] = array(
					'login'        => $values['login'],

...
function xNewsShowBlock($params) {
	...
	// skipCurrent
	if (isset($params['skipCurrent']) and $params['skipCurrent'] and (($CurrentHandler['handlerName'] == 'news') || ($CurrentHandler['handlerName'] == 'print'))) {
	...
	if (($showResult['count'] < 1) and (isset($params['showNoNews']) || ($params['showNoNews']))) {
		$xOut = '';
	}
	if (!$cacheDisabled and ($params['cacheAge'] > 0)) {

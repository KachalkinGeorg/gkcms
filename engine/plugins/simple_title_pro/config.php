...
switch ($_REQUEST['action']) {
	...
	case 'del_cat':			del_cat();		break;
	case 'del_static':		del_static();	break;
...
function about()
{global $twig, $breadcrumb, $lang;
	$tpath = locatePluginTemplates(array('main', 'about'), 'simple_title_pro', 1);
	
	$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Simple Title Pro</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-paragraph btn-position"></i>Simple Title Pro' ) );
...
{global $userROW;
	...
	msg(array("type" => "info", "info" => $userROW['name']." Кэш очишен"));
	return print_msg( 'cache', 'Simple Title Pro', 'Очистка кэша завершена успешно!', 'javascript:history.go(-1)' );
}
...
{global $twig, $mysql, $lang, $breadcrumb;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Список новостей</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Список новостей' ) );
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM '.prefix.'_simple_title_pro as s LEFT JOIN '.prefix.'_news as n ON (n.id = s.news_id) WHERE s.news_id != 0 ORDER BY s.id ASC LIMIT '.$start_from.', '.$news_per_page) as $row){
		...
		$tVars = array (
			'title' => $row['stitle'],
			'id' => $row['sid'],
			'modal' => print_modal_dialog($row['sid'], 'Удалить '.$row['stitle'].'', 'Вы уверены, что хотите удалить?', '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_news&id='.$row['sid'].'" class="btn btn-outline-success">да</a>'),
			'name' => '<a href="?mod=extra-config&plugin=simple_title_pro&action=send_title&do=news&edit='.$row['sid'].'"  />'.$row['name'].'</a>',
			'del' => '<a href="#" data-toggle="modal" data-target="#modal-'.$row['sid'].'" /><img title="Удалить" alt="Удалить" src="/engine/skins/default/images/delete.gif"></a>',
		);
...
{global $twig, $mysql, $lang, $breadcrumb;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Список статистических статей</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Список статистических страниц' ) );
...
{global $twig, $mysql, $lang, $breadcrumb;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Список категорий</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Список категорий' ) );
...
function send_title()
{global $twig, $mysql, $config, $lang, $breadcrumb;
	...
	switch($_REQUEST['do']){
		case 'cat':
			...
			if($_REQUEST['edit']){
				$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">' . $frow['title'] . '</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Редактирование '.$frow['title'].'' ) );
			}else{
				$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Добавить ТИТЛ в категорию</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Добавить ТИТЛ в категорию' ) );
			}
			...
			if (isset($_REQUEST['submit'])){
				...
				if(empty($error_text) && empty($_REQUEST['edit'])){
					...
					');
					msg(array("type" => "info", "info" => "Запись добавлена"));
					return print_msg( 'success', 'Simple Title Pro', 'Категория успешно добавлена!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=cat' => 'Добавить еще', '?mod=extra-config&plugin=simple_title_pro&action=list_cat' => 'Вернуться назад' ) );
					...
					');
					msg(array("type" => "info", "info" => "Запись отредактированна"));
					return print_msg( 'update', 'Simple Title Pro', 'Категория '.$title.' успешно отредактированна!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=cat&edit='.$id => 'Редактировать еще', '?mod=extra-config&plugin=simple_title_pro&action=list_cat' => 'Вернуться назад' ) );
		...
		case 'news':
			...
			if($_REQUEST['edit']){
				...
				$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Добавить ТИТЛ в новость</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Добавить ТИТЛ в новость' ) );
			}
			...
			if (isset($_REQUEST['submit'])){
				...
				if(empty($error_text) && empty($_REQUEST['edit'])){
					...
					');
					msg(array("type" => "info", "info" => "Запись добавлена"));
					return print_msg( 'success', 'Simple Title Pro', 'Новость успешно добавлена!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=news' => 'Добавить еще', '?mod=extra-config&plugin=simple_title_pro&action=list_news' => 'Вернуться назад' ) );
				}else if(empty($error_text) && !empty($_REQUEST['edit'])){
					...
					');
					msg(array("type" => "info", "info" => "Запись отредактированна"));
					return print_msg( 'update', 'Simple Title Pro', 'Новость '.$title.' успешно отредактированна!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=news&edit='.$id => 'Редактировать еще', '?mod=extra-config&plugin=simple_title_pro&action=list_news' => 'Вернуться назад' ) );
		...
		case 'static':
			...
			if($_REQUEST['edit']){
				...
				$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Добавить ТИТЛ в статик</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=simple_title_pro' => '<i class="fa fa-puzzle-piece btn-position"></i>Simple Title Pro', 'Добавить ТИТЛ в статик' ) );
			}
			...
			if (isset($_REQUEST['submit'])){
				...
				if(empty($error_text) && empty($_REQUEST['edit'])){
					...
					');
					msg(array("type" => "info", "info" => "Запись добавлена"));
					return print_msg( 'success', 'Simple Title Pro', 'Статистическая страница успешно добавлена!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=static' => 'Добавить еще', '?mod=extra-config&plugin=simple_title_pro&action=list_static' => 'Вернуться назад' ) );
				}else if(empty($error_text) && !empty($_REQUEST['edit'])){
					...
					$mysql->query('UPDATE '.prefix.'_simple_title_pro SET 
						...
						WHERE id = \''.intval($_REQUEST['edit']).'\'
					');
					msg(array("type" => "info", "info" => "Запись отредактирована"));
					return print_msg( 'update', 'Simple Title Pro', 'Статистическая страница '.$title.' успешно отредактированна!', array('?mod=extra-config&plugin=simple_title_pro&action=send_title&do=static&edit='.$id => 'Редактировать еще', '?mod=extra-config&plugin=simple_title_pro&action=list_static' => 'Вернуться назад' ) );
				}
			}
		break;
		default: redirect('?mod=extra-config&plugin=simple_title_pro');
	}
...
{global $twig, $lang, $breadcrumb;
	...
	if (isset($_REQUEST['submit'])){
		...
		pluginSetVariable('simple_title_pro', 's_title', secure_html(trim($_REQUEST['s_title'])));
		...
		pluginSetVariable('simple_title_pro', 'd_title', secure_html(trim($_REQUEST['d_title'])));
		...
		pluginSetVariable('simple_title_pro', 'separator', secure_html(trim($_REQUEST['separator'])));
		pluginsSaveConfig();
		msg(array("type" => "info", "info" => "Настройки успешно сохранены!"));
		return print_msg( 'info', 'Simple Title Pro', 'Настройки успешно сохранены!', '?mod=extra-config&plugin=simple_title_pro' );
	}
	...
	$separator = pluginGetVariable('simple_title_pro', 'separator');
	...
	$tVars = array(
		'c_title' => array(
						'print' => $c_title,
						'error' => empty($c_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %s% %cat% [%s% %num%]':''
		),
		'n_title' => array(
						'print' => $n_title,
						'error' => empty($n_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home%  %s% %cat% %s% %title% [%s% %num%]':''
		),
		'm_title' => array(
						'print' => $m_title,
						'error' => empty($m_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %num%':''
		),
		's_title' => array(
						'print' => $s_title,
						'error' => empty($s_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %s% %static%':''
		),
		'o_title' => array(
						'print' => $o_title,
						'error' => empty($o_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %s% %other% %html% [%s% %num%]':''
		),
		'e_title' => array(
						'print' => $e_title,
						'error' => empty($e_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %s% %other%':''
		),
		'd_title' => array(
						'print' => $d_title,
						'error' => empty($d_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %home% %s% %other%':''
		),
		'num_title' => array(
						'print' => $num_title,
						'error' => empty($num_title)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %s% Страница %count%':''
		),
		'num_cat' => array(
						'print' => $num_cat,
						'error' => empty($num_cat)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> 20':''
		),
		'num_news' => array(
						'print' => $num_news,
						'error' => empty($num_news)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> 20':''
		),
		'num_static' => array(
						'print' => $num_static,
						'error' => empty($num_static)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> 20':''
		),
		'html_secure' => array(
						'print' => $html_secure,
						'error' => empty($html_secure)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> %s% %html%':''
		),
		'p_title' => array(
						'print' => $p_title,
						'error' => ''
		...
		'cache' => array(
						'print' => $cache,
						'error' => empty($cache)?'<img src="'.skins_url.'/images/error.gif" hspace="5" alt="" />Поле не заполнено! <b>Ремомендованно:</b> 1':''
		),
	);
	...
	foreach($tVars as $row){
		...
		if(!empty($row['error'])){
			...
			msg(array("type" => "error", "info" => 'Внимание! У Вас не заполнено одно из полей, проверьте правильность ввода данных.'));
			break;
		}
	}
...
{global $mysql;
	...
	return print_msg( 'delete', 'Simple Title Pro', 'Выбранная категория была успешна удалена!', 'javascript:history.go(-1)' );
...
{global $mysql;
	...
	$mysql->query("delete from ".prefix."_simple_title_pro where id = {$id} LIMIT 1");
	msg(array("type" => "info", "info" => "Запись удалена"));
	return print_msg( 'delete', 'Simple Title Pro', 'Выбранная запись новости была успешна удалена!', 'javascript:history.go(-1)' );
...
{global $mysql;
	...
	$mysql->query("delete from ".prefix."_simple_title_pro where id = {$id} LIMIT 1");
	msg(array("type" => "info", "info" => "Запись удалена"));
	return print_msg( 'delete', 'Simple Title Pro', 'Выбранная статья была успешна удалена!', 'javascript:history.go(-1)' );
}
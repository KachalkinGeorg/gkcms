...
function uprofile_showProfile($params) {
	...
	$tVars = array(
		...
		'user'    => array(
			'id'          => $urow['id'],
			'name'        => $urow['name'],
			'alt_name'    => $urow['alt_name'] ? $urow['alt_name'] : 'нет данных',
			'news'        => $urow['news'],
			'com'         => $urow['com'],
			'status'      => $status,
			'line'     	  => on_of_line($urow['id']),
			'last'        => ($urow['last'] > 0) ? LangDate("l, j Q Y - H:i", $urow['last']) : $lang['uprofile']['no_last'],
			'reg'         => langdate("j Q Y", $urow['reg']),
			'gender'	  => $lang['uprofile']['gender_' . $urow['gender']],
	...
	if (pluginGetVariable('uprofile', 'profilegrup')) {
		$grup = '_'.$urow['status'].'';
	}else{
		$grup = '';
	}
	$twigLoader->setConversion($tpath['users'] . 'users.tpl', $conversionConfig, $conversionConfigRegex);
	$xt = $twig->loadTemplate($tpath['users'] . 'users'.$grup.'.tpl');
...
function uprofile_editForm($ajaxMode = false) {
	...
	$tVars = array(
		...
		'user'                => array(
			'id'          => $urow['id'],
			'name'        => $urow['name'],
			'alt_name'    => $urow['alt_name'],
			'news'        => $urow['news'],
			'com'         => $urow['com'],
			'status'      => $status,
			'last'        => ($urow['last'] > 0) ? LangDate("l, j Q Y - H:i", $urow['last']) : $lang['no_last'],
			'reg'         => langdate("j Q Y", $urow['reg']),
			'gender'      => makeDropDown(array('0' => "Нет определения", '1' => "Мужской", '2' => "Женский"), 'gender', $urow['gender']),
	...
	$twigLoader->setConversion($tpath['profile'] . 'profile'.$grup.'.tpl', $conversionConfig, $conversionConfigRegex);
	$xt = $twig->loadTemplate($tpath['profile'] . 'profile'.$grup.'.tpl');

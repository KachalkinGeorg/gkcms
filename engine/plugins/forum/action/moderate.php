...
switch ($metod) {
	...
	case 'fixap':
		if (moder_perm($rows['tid'], 'topic_move', $row_['moderators']) || $userROW['status'] == 1) {
			$mysql->query('UPDATE ' . prefix . '_forum_posts SET fixed = \'1\' WHERE tid = ' . securemysql($tid) . ' LIMIT 1');
			$mysql->query('UPDATE ' . prefix . '_forum_topics SET fixed = \'1\' WHERE id = ' . securemysql($tid) . ' LIMIT 1');
		}
	...
	case 'unfixap':
		if (moder_perm($rows['tid'], 'topic_move', $row_['moderators']) || $userROW['status'] == 1) {
			$mysql->query('UPDATE ' . prefix . '_forum_posts SET fixed = \'0\' WHERE tid = ' . securemysql($tid) . ' LIMIT 1');
			$mysql->query('UPDATE ' . prefix . '_forum_topics SET fixed = \'0\' WHERE id = ' . securemysql($tid) . ' LIMIT 1');
		}
		return redirect_forum(link_topic($tid));
		break;

...
if (is_array($userROW))
	$result = $mysql->record('SELECT t.id as tid, t.title as Ttitle, t.author_id, t.state, t.pinned, t.fixed, t.int_post, f.id as fid, f.title as Ftitle, f.moderators, f.lock_passwd, f.description, f.keywords, f.redirect_url, s.uid 
	...
	$result = $mysql->record('SELECT t.id as tid, t.title as Ttitle, t.author_id, t.state, t.pinned, t.fixed, t.int_post, f.id as fid, f.title as Ftitle, f.moderators, f.lock_passwd, f.description, f.keywords, f.redirect_url 
				FROM ' . prefix . '_forum_topics AS t 
				INNER JOIN ' . prefix . '_forum_forums AS f ON f.id = t.fid 
				WHERE t.id = ' . securemysql($id) . ' LIMIT 1');
if (empty($result))
...
$twig->addGlobal('fixed', $result['fixed']);
...
if (is_array($userROW)){
	$metadescription = " ". $result['description']." - ". $result['Ttitle'] ."". $s."";
} else {
	$metadescription = "Форум - ". $result['description']."";
}
$SYSTEM_FLAGS['meta']['description'] = $metadescription;
$SYSTEM_FLAGS['meta']['keywords'] = $result['keywords'];
...
$result_2 = $mysql->select('SELECT p.id as pid, p.message, p.author_id, p.author, p.tid, 
					p.e_date, p.who_e_author, p.author_ip, p.c_data, p.fixed,
				...
				WHERE p.tid = ' . securemysql($id) . ' ' . $search . ' AND p.fixed = 0 ORDER BY p.c_data ASC 
...
foreach ($mysql->select('SELECT p.id as pid, p.message, p.author_id, p.author, p.tid, 
					p.e_date, p.who_e_author, p.author_ip, p.c_data,
					u.reg, u.id as uid, u.name, u.int_post, u.avatar, u.reputation, u.site, u.last, u.status, u.signature, u.mail, u.int_thank
				FROM ' . prefix . '_forum_posts AS p 
				LEFT JOIN ' . prefix . '_users AS u ON u.id = p.author_id 
				WHERE p.tid = ' . securemysql($id) . ' AND p.fixed = 1 ORDER BY p.c_data DESC') as $row) {
	$i++;
	...
	$editdat = pluginGetVariable('forum', 'editdate');
	if ($editdat) {
	$editdate = array(
	...
	$tEntryFix[] = array(
		...
		'topic_links'     => link_topic($row['tid']),
		'page'     		  => '&page='.$pageNo.'',
...
$tVars = array(
	...
	'entries_fix'      => isset($tEntryFix) ? $tEntryFix : '',
	'pages'        => array(

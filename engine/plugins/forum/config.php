...
function main($entries) {
	...
	if (!file_exists(files_dir . 'forum')){
		...
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: не найдена папка ' . files_dir . 'forum'));
	}
	if (!is_writable(files_dir . 'forum'))
		//$_SESSION['forum']['info'][] = 'Критическая ошибка: нет прав на запись ' . files_dir . 'forum';
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: нет прав на запись ' . files_dir . 'forum'));
	if (!is_writable(FORUM_CACHE)){
		//$_SESSION['forum']['info'][] = 'Критическая ошибка: не найдена папка ' . FORUM_CACHE;
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: не найдена папка ' . FORUM_CACHE));
	}
	if (!is_writable(FORUM_CACHE)){
		//$_SESSION['forum']['info'][] = 'Критическая ошибка: нет прав на запись ' . FORUM_CACHE;
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: нет прав на запись ' . FORUM_CACHE));
	}
	if (file_exists(FORUM_CACHE) && !is_writable(FORUM_CACHE . '/group_perm.php')){
		...
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: нет прав на запись ' . FORUM_CACHE . '/group_perm.php'));
	}
	if (file_exists(FORUM_CACHE) && !is_writable(FORUM_CACHE . '/forum_perm.php')){
		...
		msg(array('type' => 'error', 'text' => 'Критическая ошибка: нет прав на запись ' . FORUM_CACHE . '/forum_perm.php'));
	}
...
function edit_group() {
	...
	global $plugin, $twig, $mysql, $lang, $breadcrumb;
	$id = intval($_REQUEST['id']);
	if (!isset($id) & $id <> '')
		redirect('?mod=extra-config&plugin=forum&action=group');
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			return print_msg( 'info', 'Форум', 'Настройки успешно сохранены!', '?mod=extra-config&plugin=forum&action=group' );
		}
	}
	$error_input = array();
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Редактирование '.$group_name.'' ) );
	$entries_main = array(
...
function ban() {
	...
	global $plugin, $twig, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('ban'), $plugin, 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Забаненые тест страницы' ) );
...
function ban_ip_list() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Список забаненых IP' ) );
...
function group() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Группы' ) );
...
function about() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'О плагине' ) );
...
function closed_complaints() {
	...
	if (isset($id) && $id) {
		...
		msg(array("type" => "info", "info" => "Жалоба закрыта"));
		return print_msg( 'info', 'Форум', 'Жалоба закрыта!', '?mod=extra-config&plugin=forum&action=list_complaints' );
	} else {
		//$_SESSION['forum']['info'] = 'Ошибка';
		msg(array("type" => "error", "text" => "Ошибка ввода вывода."));
		return print_msg( 'warning', 'Форум', 'Внимание!<br>Неверные значения переменных! Ошибка ввода вывода.', '?mod=extra-config&plugin=forum' );
	}
}

function list_complaints() {

	global $twig, $plugin, $config, $mysql, $lang, $breadcrumb;
	if (isset($_SESSION['forum']['info'])) {
		$info = $_SESSION['forum']['info'];
		session_destroy();
	}
	$tpath = locatePluginTemplates(array('complaints'), $plugin, 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Список жалоб' ) );
...
function del_forum() {
	...
	if (!$mysql->result('SELECT 1 FROM ' . prefix . '_forum_topics WHERE fid = ' . db_squote($id) . ' LIMIT 1')) {
		...
		msg(array("type" => "info", "info" => "Под форум удален"));
		generate_index_cache(true);
		return print_msg( 'delete', 'Форум', 'Выбранный под форум был успешно удален', 'javascript:history.go(-1)' );
	} else {
		//$_SESSION['forum']['info'] = 'Форум не удален';
		msg(array("type" => "info", "info" => "Под форум не удален"));
		return print_msg( 'warning', 'Форум', 'Внимание!<br>Выбранный Вами под форум не удален!', '?mod=extra-config&plugin=forum&action=list_forum' );
	}
	redirect('?mod=extra-config&plugin=forum&action=list_forum');
}
...
function del_section() {
	...
	if (!$mysql->result('SELECT 1 FROM ' . prefix . '_forum_forums WHERE parent = ' . db_squote($id) . ' LIMIT 1')) {
		...
		msg(array("type" => "info", "info" => "Раздел удален"));
		generate_index_cache(true);
		return print_msg( 'delete', 'Форум', 'Выбранный раздел форума был успешно удален!', 'javascript:history.go(-1)' );
	} else {
		msg(array("type" => "info", "info" => "Нельзя удалять раздел с форумом"));
		return print_msg( 'warning', 'Форум', 'Внимание!<br>Нельзя удалять цулевой Раздел форума, который содержит в себе уже форумы!', '?mod=extra-config&plugin=forum&action=list_forum' );
	}
	redirect('?mod=extra-config&plugin=forum&action=list_forum');
}
...
function edit_section() {
	...
	global $twig, $plugin, $mysql, $lang, $breadcrumb;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Редактирование '.$name.'' ) );
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			msg(array("type" => "info", "info" => "Раздел форума отредактирован!"));
			generate_index_cache(true);
			return print_msg( 'update', 'Форум', 'Раздел форума '.$name.' успешно отредактирован!', array('?mod=extra-config&plugin=forum&action=edit_section&id'.$id => 'Редактировать еще', '?mod=extra-config&plugin=forum&action=list_forum' => 'Вернуться назад' ) );
		}
	}
	$error_input = array();
	if (isset($error_text) && is_array($error_text))
...
function send_section() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Добавить раздел' ) );
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			');
			msg(array("type" => "info", "info" => "Раздел форума добавлен"));
			generate_index_cache(true);
			return print_msg( 'success', 'Форум', 'Раздел форума успешно создан!', '?mod=extra-config&plugin=forum&action=list_forum' );
		}
	}
	$error_input = array();
	if (isset($error_text) && is_array($error_text))
...
function edit_forum() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Редактирование '.$forum_name.'' ) );
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			msg(array("type" => "info", "info" => "Под форум отредактирован"));
			return print_msg( 'update', 'Форум', 'Под форум '.$forum_name.' успешно отредактирован!', array('?mod=extra-config&plugin=forum&action=edit_forum&id='.$id => 'Редактировать еще', '?mod=extra-config&plugin=forum&action=list_forum' => 'Вернуться назад' ) );
		}
	}
...
function send_forum() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Добавить форум' ) );
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			file_put_contents(FORUM_CACHE . '/mode_perm.php', '<?php' . "\n\n" . 'if (!defined(\'NGCMS\')) die (\'HAL\');' . "\n\n" . '$MODE_PERM = ' . var_export($result, true) . ';' . "\n\n");
			generate_index_cache(true);
			return print_msg( 'info', 'Форум', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
		}
	}
	//print "<pre>".var_export($forum_moderators, true)."</pre>";
	foreach ($mysql->select('SELECT * from ' . prefix . '_forum_group') as $row) {
...
function title() {
	...
	global $twig, $plugin, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('title'), 'forum', 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Управление заголовками' ) );
	if (isset($_REQUEST['submit'])) {
		...
		pluginsSaveConfig();
		msg(array("type" => "info", "info" => "Настройки сохранены успешно!"));
		return print_msg( 'info', 'Форум', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
	}
...
function url() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'ЧПУ' ) );
	if ($_REQUEST['id']) {
		$ULIB = new urlLibrary();
		$ULIB->loadConfig();
		...
		$ULIB->saveConfig();
		$UHANDLER->saveConfig();
		redirect('?mod=extra-config&plugin=forum&action=url');
	}
...
function moderat() {
	...
	switch ($_REQUEST['act']) {
		case 'pinned':
			...
			//redirect(link_forum($_REQUEST['tid']));
			break;
	}
}
...
function rules() {
	...
	if (isset($_REQUEST['submit'])) {
		...
		pluginsSaveConfig();
		msg(array("type" => "info", "info" => "Правила форума добавлены"));
		return print_msg( 'info', 'Форум', 'Правила форума успешно созданы!', '?mod=extra-config&plugin=forum&action=rules' );
	}
	$tpath = locatePluginTemplates(array('rules', ':'), 'forum', 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Правила' ) );
...
function ads() {
	...
	global $twig, $plugin, $lang, $breadcrumb;
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Объявления' ) );
	if (isset($_REQUEST['submit'])) {
		...
		pluginsSaveConfig();
		msg(array("type" => "info", "info" => "Объявление добавлено"));
		return print_msg( 'info', 'Форум', 'Правила форума успешно созданы!', '?mod=extra-config&plugin=forum&action=ads' );
	}
...
function new_news() {
	...
	global $twig, $plugin, $mysql, $SYSTEM_FLAGS, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('new_news', 'htmail', ':'), 'forum', 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Список новостей' ) );
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			...
			redirect('?mod=extra-config&plugin=forum&action=list_news');
		}
	}
...
function del_news() {
	...
	msg(array("type" => "info", "info" => "Новость удалена"));
	return print_msg( 'delete', 'Форум', 'Новость был успешна удалена!', 'javascript:history.go(-1)' );
}
...
function list_news() {
	...
	foreach ($mysql->select('SELECT * FROM ' . prefix . '_forum_news ORDER BY c_data DESC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		$tEntry[] = array(
			...
			'title'   => $row['title'],
			'content' => $row['content'],
			'edit'    => '<a href="?mod=extra-config&plugin=forum&action=new_news&id=' . $row['id'] . '";><i class="fa fa-pencil-square-o"></i></a>',
			'del'     => '<a href="#" data-toggle="modal" data-target="#modal-' . $row['id'] . '"><i style="color:#ed143d" class="fa fa-trash"></i></a>',
		);
	}
	$tVars = array(
		'entries' => isset($tEntry) ? $tEntry : '',
...
function list_forum() {
	...
	global $twig, $plugin, $mysql, $config, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('list_forum', 'list_forum_entries', 'list_forum_main'), 'forum', 1, '', 'config');
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Список форумов' ) );
	if (isset($_SESSION['forum']['info'])) {
		$info = $_SESSION['forum']['info'];
		session_destroy();
	}
...
function general() {
	...
	global $twig, $plugin, $lang, $breadcrumb;
	$breadcrumb = breadcrumb('<i class="fa fa-rocket btn-position"></i><span class="text-semibold">Форум</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=forum' => '<i class="fa fa-rocket btn-position"></i>Форум',  'Главная' ) );
	if (isset($_REQUEST['submit'])) {
		...
		pluginSetVariable('forum', 'forum_disc', (int)$_REQUEST['forum_disc']);
		pluginSetVariable('forum', 'compost', (int)$_REQUEST['compost']);
		pluginSetVariable('forum', 'editdate', (int)$_REQUEST['editdate']);
		pluginSetVariable('forum', 'forum_but1', secure_html(trim($_REQUEST['forum_but1'])));
		pluginSetVariable('forum', 'forum_but2', secure_html(trim($_REQUEST['forum_but2'])));
		pluginsSaveConfig();
	...
	$tVars = array(
		...
		'forum_but1'  		=> secure_html(trim(pluginGetVariable($plugin, 'forum_but1'))),
		'forum_but2'  		=> secure_html(trim(pluginGetVariable($plugin, 'forum_but2'))),
		'forum_disc'        => MakeDropDown(array(0 => 'Нет', 1 => 'Да'), 'forum_disc', (int)pluginGetVariable($plugin, 'forum_disc')),
		'compost'           => MakeDropDown(array(0 => 'Нет', 1 => 'Да'), 'compost', (int)pluginGetVariable($plugin, 'compost')),
		'editdate'          => MakeDropDown(array(0 => 'Нет', 1 => 'Да'), 'editdate', (int)pluginGetVariable($plugin, 'editdate')),
	);
	$entries_main = array(
		'show' => $xg->render($tVars),
		'mode' => 'Общие'

...
function bb_codes($text) {
	...
	$bb_open[] = '/\[h6\](.*?)\[\/h6\]/is';
	$bb_close[] = '<h6>\\1</h6>';
	$bb_open[] = '/\[center\](.*?)\[\/center\]/is';
	$bb_close[] = '<center>\\1</center>';
	$bb_open[] = '/\[left\](.*?)\[\/left\]/is';
	$bb_close[] = '<p style="text-align: left">\\1</p>';
	$bb_open[] = '/\[right\](.*?)\[\/right\]/is';
	$bb_close[] = '<p style="text-align: right">\\1</p>';
	$bb_open[] = '/\[hr\]/is';
	$bb_close[] = '<hr/>';
	$bb_open[] = '/\[br\]/is';
	$bb_close[] = '<br/>';
	$bb_open[] = '/\[code\](.*?)\[\/code\]/is';
	$bb_close[] = '<pre><code>\\1</code></pre>';
...
/* 	$bb_close_f[] = array('forum_open_quote_tag');
    ...
    while (preg_match("#\[spoiler\](.*?)\[/spoiler\]#isu", $text)) {
        $text = preg_replace("#\[spoiler\](.*?)\[/spoiler\]#is", '<div class="spoiler" tabindex="1"><div class="sp-head" onclick="toggleSpoiler(this.parentNode, this);"><b></b>Раскрыть</div><div class="sp-body">$1</div></div>', $text);
    }

    while (preg_match("#\[spoiler=\"(.+?)\"\](.*?)\[/spoiler\]#isu", $text)) {
        $text = preg_replace("#\[spoiler=\"(.+?)\"\](.*?)\[/spoiler\]#is", '<div class="spoiler"><div class="sp-head" onclick="toggleSpoiler(this.parentNode, this);"><b></b>$1</div><div class="sp-body">$2</div></div>', $text);
    }
...
function forum_img_tag($url, $text) {
	...
	if (empty($url)) return;
	$url = secureinput($url);
	$width = "style=\"width:100%;max-width:279px;\"";

	return '<a href="'.$url.'" rel="nofollow" onclick="return hs.expand (this)" class="highslide"><img src=\'' . $url . '\' border=\'0\' ' . $width . ' alt=\'' . $text . '\' /></a>';
}

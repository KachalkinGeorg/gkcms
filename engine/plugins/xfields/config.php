...
function doAddEdit()
{
    ...
    if ($editMode && (!is_array($xf[$sectionID][$field]))) {
		...
		return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_noexists'], 'javascript:history.go(-1)' );
        $error = 1;
    } elseif (!$editMode && (is_array($xf[$sectionID][$field]))) {
        msg(['type' => 'error', 'text' => $lang['xfields_msge_exists']]);
		return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_exists'], 'javascript:history.go(-1)' );
        $error = 1;
    }
    ...
    if (!$editMode) {
        if (!preg_match('/^[a-z]{1}[a-z0-9]{2}[a-z0-9]*$/', $field)) {
			...
			return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_format'], 'javascript:history.go(-1)' );
            $error = 1;
        }
    }
            ...
            */
            ...
            if (trim($_REQUEST['select_default'])) {
                ...
                if (
					...
					return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_errdefault'], 'javascript:history.go(-1)' );
                    $error = 1;
                }
            }
            break;
    ...
    if (!$data['type']) {
		...
		return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_errtype'], 'javascript:history.go(-1)' );
        $error = 1;
    }
    if (!$data['title']) {
        msg(['type' => 'error', 'text' => $lang['xfields_msge_errtitle']]);
		return print_msg( 'error', 'Дополнительные поля', $lang['xfields_msge_errtitle'], 'javascript:history.go(-1)' );
        $error = 1;
    }
    ...
    if ($data['storage']) {
        ...
        if (!in_array($data['db.type'], ['int', 'decimal', 'char', 'datetime', 'text'])) {
			...
			return print_msg( 'error', 'Дополнительные поля', $lang['xfields_error.db.type'], 'javascript:history.go(-1)' );
            $error = 1;
        }
        ...
        if (($data['db.type'] == 'char') && ((intval($data['db.len']) < 1) || (intval($data['db.len']) > 255))) {
			...
			return print_msg( 'error', 'Дополнительные поля', $lang['xfields_error.db.len'], 'javascript:history.go(-1)' );
            $error = 1;
        }
    }
    if ($error) {
...
function doUpdate()
{
    ...
    if (!is_array($xf[$sectionID][$field])) {
		...
		return print_msg( 'error', 'Дополнительные поля', ''.$lang['xfields_msge_noexists'].' ('.$sectionID.': '.$field.').', 'javascript:history.go(-1)' );
        $error = 1;
    }
    $notif = '';

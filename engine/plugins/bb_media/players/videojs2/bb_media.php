...
function bbMediaInclude() {
	...
	register_htmlvar('css', admin_url . '/plugins/bb_media/players/videojs2/lib/video-js.css');
	register_htmlvar('css', admin_url . '/plugins/bb_media/players/videojs2/lib/video-js.min.css');
	register_htmlvar('js', admin_url . '/plugins/bb_media/players/videojs2/lib/video.cjs.js');
	register_htmlvar('js', admin_url . '/plugins/bb_media/players/videojs2/lib/video.min.js');
	register_htmlvar('js', admin_url . '/plugins/bb_media/players/videojs2/lib/lang/ru.js');
	register_htmlvar('js', admin_url . '/plugins/bb_media/players/videojs2/lib/video.js');
	//register_htmlvar('plain', '<script> videojs.options.flash.swf = "' . admin_url . '/plugins/bb_media/players/videojs2/lib/video-js.swf"; </script>');
	register_htmlvar('js', admin_url . '/plugins/bb_media/players/videojs2/lib/plugins/youtube/youtube.min.js');
}
...
function bbMediaProcess($content) {
	...
	if (preg_match_all("#\[media(\=| *)(.*?)\](.*?)\[\/media\]#is", $content, $pcatch, PREG_SET_ORDER)) {
		...
		foreach ($pcatch as $catch) {
			...
			// - preview image
			if (isset($keys['preview']) && preg_match("#^https\:\/\/.*?\.(png|jpg|gif|webp)$#i", $keys['preview'], $m)) {

...
switch ($_REQUEST['action']) {
	...
	case 'clear_cash':
		clear_cash();
		print_msg( 'cache', 'ADS manager pro', 'Очистка кэща завершена успешно', 'javascript:history.go(-1)' );
		break;
	default:
		main();
}
function main() {
	...
	global $tpl, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('conf.main', 'conf.general'), 'ads_pro', 1);
	$breadcrumb = breadcrumb('<i class="fa fa-television btn-position"></i><span class="text-semibold">ADS manager pro</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-television btn-position"></i>ADS manager pro' ) );
	...
	$ttvars['vars'] = array(
		...
		'panel'              	=> $lang['ads_pro:button_general'],
	...
	$tvars['vars']['panel'] = $lang['ads_pro:button_general'];
...
function main_submit() {
	...
	return print_msg( 'info', 'ADS manager pro', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
}
...
function showlist() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-television btn-position"></i><span class="text-semibold">ADS manager pro</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=ads_pro' => '<i class="fa fa-television btn-position"></i>ADS manager pro',  'Список блоков' ) );
	...
	$tvars['vars']['panel'] = $lang['ads_pro:button_list'];
	$tpl->template('conf.main', $tpath['conf.main']);
	$tpl->vars('conf.main', $tvars);
	print $tpl->show('conf.main');
}
...
function add() {
	...
	global $mysql, $tpl, $lang, $breadcrumb;
	...
	if($id){
		$addedit = 'Редактировать - '.$name.'';
	}else{
		$addedit = 'Добавить';
	}
	$breadcrumb = breadcrumb('<i class="fa fa-television btn-position"></i><span class="text-semibold">ADS manager pro</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=ads_pro' => '<i class="fa fa-television btn-position"></i>ADS manager pro',  ''.$addedit.'' ) );
	...
	$tvars['vars']['panel'] = $id ? $lang['ads_pro:button_edit'] : $lang['ads_pro:button_add'];
	$tpl->template('conf.main', $tpath['conf.main']);
	$tpl->vars('conf.main', $tvars);
	print $tpl->show('conf.main');
}
...
function add_submit() {
	...
	if (!$id) {
		...
		msg(array('type' => 'info', 'info' => "Блок успешно добавлен!"));
		print_msg( 'success', 'ADS manager pro', 'Новый блок успешно создан!', '?mod=extra-config&plugin=ads_pro&action=list' );
	} else {
		$t_update = $mysql->query("update " . prefix . "_ads_pro set ads_blok=" . db_squote($ads_blok) . " where id=" . db_squote($id) . " limit 1");
		msg(array('type' => 'info', 'info' => "Блок успешно отредактирован!"));
		print_msg( 'update', 'ADS manager pro', 'Блок успешно отредактирован!', '?mod=extra-config&plugin=ads_pro&action=list' );
		$t_name = 0;
		$if_brek = false;
		foreach ($var as $k => $v) {
			foreach ($v as $kk => $vv) {
...
function delete() {
	...
	clear_cash();
	print_msg( 'delete', 'ADS manager pro', 'Удаление прошло успешно!', '?mod=extra-config&plugin=ads_pro&action=list' );
}

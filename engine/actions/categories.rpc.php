...
// Description: Библиотека RPC для манипуляций категорий
...
{
    global $mysql, $PHP_SELF, $twig, $config, $lang, $AFILTERS, $breadcrumb;
...
function admCategoriesRPCmodify($params)
{
    ...
    if (!checkPermission(['plugin' => '#admin', 'item' => 'categories'], null, 'modify')) {
        // ACCESS DENIED
        return ['status' => 0, 'errorCode' => 3, 'errorText' => $lang['msge_no_acces']];
    }

    // Scan incoming params
    if (!is_array($params) || !isset($params['mode']) || !isset($params['id']) || !isset($params['token'])) {
        return ['status' => 0, 'errorCode' => 4, 'errorText' => $lang['msge_no_type']];
    }

    // Check for security token
    if ($params['token'] != genUToken('admin.categories')) {
        return ['status' => 0, 'errorCode' => 5, 'errorText' => $lang['msge_no_cod']];
    }

    // Check if category exists
    if (!isset($catmap[$params['id']])) {
        return ['status' => 0, 'errorCode' => 10, 'errorText' => $lang['msge_no_cat']];
    }
    ...
    switch ($params['mode']) {
        ...
        case 'del':
            ...
            if ($refCCount['cnt'] > 0) {
                return ['status' => 0, 'errorCode' => 11, 'errorText' => $lang['msge_havechildren']];
            }
            ...
            if ($refNCount['cnt'] > 0) {
                return ['status' => 0, 'errorCode' => 12, 'errorText' => $lang['msge_havenews']];
            }

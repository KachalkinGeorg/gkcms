...
{
    global $lang, $parse, $mysql, $config, $PFILTERS, $tvars, $userROW, $twig, $PHP_SELF, $breadcrumb, $UGROUP;
...
/*     foreach ($UGROUP as $k => $v) {
    ...
    $tVars = [
        ...
        'flags'       => [
			...
			'add_info'            => $config['news.add.info'] ? true : false,
			'add_scrin'           => $config['news.add.scrin'] ? true : false,
			...
			'nosearch'            => $row['nosearch'] ? true : false,
			'fixed'               => $row['fixed'] ? true : false,
			'robots'              => $row['robots'] ? true : false,
			...
			'fixed.lost'          => (($row['flags'] & 2) && (!$perm[$permGroupMode.'.fixed'])) ? 1 : 0,
			...
			'nosearch.disabled'   => (!$perm[$permGroupMode.'.nosearch']) ? true : false,
			'fixed.disabled' 	  => (!$perm[$permGroupMode.'.fixed']) ? true : false,
...
{
    global $mysql, $lang, $twig, $catz, $catmap, $userROW, $PHP_SELF, $config, $breadcrumb;
...
{
    global $lang, $mysql, $config, $userROW, $PFILTERS, $twig, $PHP_SELF, $breadcrumb, $UGROUP;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-file-text btn-position"></i><span class="text-semibold">'.$lang['addnews']['addnews_title'].'</span>', array('?mod=news' => '<i class="fa fa-file-text-o btn-position"></i>'.$lang['addnews']['news_title'].'', '<i class="fa fa-plus-square-o"></i>'.$lang['addnews']['addnews_title'].'' ) );

	foreach ($UGROUP as $k => $v) {
		if($k > 1)
		$usergrup .= '<input type="checkbox" name="acces['.$k.']" value="'.$k.'"> - '.$v['name'].'<br>';
	}
    ...
    $tVars = [
        ...
        'flags'      => [
			...
			'nosearch'           	  => $perm['add.nosearch'] && $perm['personal.nosearch'],
			'fixed'           	  => $perm['add.fixed'] && $perm['personal.fixed'],
			'robots'           	  => $perm['add.robots'] && $perm['personal.robots'],
			...
			'nosearch.disabled'  	  => !$perm['personal.nosearch'],
			'fixed.disabled'  	  => !$perm['personal.fixed'],
			'robots.disabled'  	  => !$perm['personal.robots'],
            'edit_split'          => $config['news.edit.split'] ? true : false,
			'add_info'            => $config['news.add.info'] ? true : false,
			'add_scrin'           => $config['news.add.scrin'] ? true : false,
            'meta'                => $config['meta'] ? true : false,

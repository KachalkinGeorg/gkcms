...
function renderDocs()
{
    global $lang, $main_admin, $twig, $breadcrumb;

    $file = $_REQUEST['file'] ?? 'about.md';

    $menu = renderMarkdown('menu.md');
    $docs = renderMarkdown($file);
	$breadcrumb = breadcrumb('<i class="fa fa-question-circle btn-position"></i><span class="text-semibold">'.$lang['docs'].'</span>', ''.$lang['docs'].'' );

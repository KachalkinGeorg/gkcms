...
function pm_read()
{
    global $lang, $userROW, $parse, $twig, $breadcrumb;
    ...
    if ($row = $db->record('select * from '.uprefix.'_users_pm where pmid = :pmid and (to_id = :to_id or from_id= :from_id)', ['pmid' => $_REQUEST['pmid'], 'to_id' => $userROW['id'], 'from_id' => $userROW['id']])) {
		...
		$breadcrumb = breadcrumb('<i class="fa fa-envelope-o btn-position"></i><span class="text-semibold">'.$lang['pm'].'</span>', array('?mod=pm' => '<i class="fa fa-envelope-o btn-position"></i>'.$lang['pm'].'', ''.$row['title'].'' ) );
...
switch ($action) {
    ...
    case 'send':
        $main_admin = pm_send();
		return print_msg( 'success', ''.$lang['pm'].'', ''.$lang['msgo_sent'].'', 'javascript:history.go(-1)' );
        break;
    case 'write':
        $main_admin = pm_write();
        break;
    case 'delete':
		...
		return print_msg( 'delete', ''.$lang['pm'].'', ''.$lang['msgo_deleted'].'', 'javascript:history.go(-1)' );
        break;
    default:
        $main_admin = pm_list();
}

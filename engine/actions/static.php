...
function listStatic()
{
    global $mysql, $mod, $userROW, $lang, $config, $twig, $PHP_SELF, $breadcrumb;
...
{
    global $lang, $parse, $mysql, $config, $twig, $mod, $PFILTERS, $tvars, $userROW, $PHP_SELF, $breadcrumb;
	...
	if ($_REQUEST['action'] == 'addForm') {
		$breadcrumb = breadcrumb('<i class="fa fa-plus btn-position"></i><span class="text-semibold">'.$lang['static_title_add'].'</span>', array('?mod=static' => '<i class="fa fa-file-text-o btn-position"></i>'.$lang['static_title'].'', '<i class="fa fa-plus-square-o"></i>'.$lang['static_title_add'].'' ) );

	}

	if ($_REQUEST['action'] == 'editForm') {
		$breadcrumb = breadcrumb('<i class="fa fa-pencil btn-position"></i><span class="text-semibold">'.$lang['static_title_edit'].'</span>', array('?mod=static' => '<i class="fa fa-file-text-o btn-position"></i>'.$lang['static_title'].'', '<i class="fa fa-pencil-square-o"></i>'.$lang['static_title_edit'].' - '.$row['title'].'' ) );
	}
...
function addStatic()
{
    ...
    if (isset($_REQUEST['flag_published']) && isset($_REQUEST['flag_published'])) {
        if (!$perm['publish']) {
			msg(['type' => 'error', 'title' => $lang['perm.denied'], 'text' => $lang['perm.publish']]);

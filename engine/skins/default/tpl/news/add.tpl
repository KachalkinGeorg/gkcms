			...
			<div class="card mb-4">
				...
				<div class="card-body">
					<div class="has-feedback" onclick="$('.cat-list').toggle();">
						<input id="catSelector" class="form-control" type="text" value="{{ lang['no_cat'] }}" hidefocus="" autocomplete="off" readonly="" style="white-space: pre-wrap;height: auto; text-align: left;">
	...
	<div id="modal-uplimg" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="uplimg-modal-label" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content" role="document">
				<div class="modal-header">
					<h5 id="uplimg-modal-label" class="modal-title">{{ lang.addnews['loading'] }}</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				</div>
				<div class="modal-body">
				
				<ul class="nav nav-tabs nav-fill" role="tablist">
					<li class="nav-item"><a href="#upl-img" class="nav-link active" data-toggle="tab">{{ lang.addnews['img_upload'] }}</a></li>
					<li class="nav-item"><a href="#upl-data" class="nav-link" data-toggle="tab">{{ lang.addnews['data_upload'] }}</a></li>
				</ul>
				...
				<div id="upl" class="tab-content">
					...
					<ul id="newsimage-area" style="list-style: none;">
						{% for image in images %}
							<li id="newsimage-item-{{ image.id }}">{{ image.entry }} <a href="#" onclick="if(confirm('Удалить')) deleteNewsImage({{ image.id }}); return false;" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a></li>
						{% endfor %}
					</ul>
					<div>
					<text style="color:#c2c0bd"><i><b>{{ lang.addnews['extra_setting'] }}</b></i></text><hr style="display:inline-block;position:relative;width:250px;top: -4px;margin: 0 6px;height: 1px;">
					<div class="form-group"></div>
					...
					{% endif %}
						<input type="checkbox" id="imageRandomTitle" name="imageRandomTitle" checked="checked"/> {{ lang.addnews['rand_name'] }}
						<div style="float: right;">
						<input type="file" id="uploadimage" name="newsimage" value=""/>
						<a class="btn btn-primary" data-placement="top" data-popup="tooltip" data-original-title="{{ lang.addnews['download'] }}" title="{{ lang.addnews['download'] }}" onclick="return uploadNewsImage();"><i class="fa fa-download"></i></a>
						</div>
					</div>
					</div>
					...
					<ul id="newsfile-area" style="list-style: none;">
						{% for file in files %}
							<li id="newsfile-item-{{ file.id }}">{{ file.entry }} <a href="#" onclick="if(confirm('Удалить')) deleteNewsFile({{ file.id }}); return false;" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a></li>
						{% endfor %}
					</ul>
					<div>
					<text style="color:#c2c0bd"><i><b>{{ lang.addnews['extra_setting'] }}</b></i></text><hr style="display:inline-block;position:relative;width:250px;top: -4px;margin: 0 6px;height: 1px;">
					<div class="form-group"></div>
					<input type="checkbox" id="fileRandomTitle" name="fileRandomTitle" checked="checked"/> {{ lang.addnews['rand_name'] }}
					<div style="float: right;">
						<input type="file" id="uploadfile" name="newsfile" value=""/>
						<a class="btn btn-primary" data-placement="top" data-popup="tooltip" data-original-title="{{ lang.addnews['download'] }}" title="{{ lang.addnews['download'] }}" onclick="return uploadNewsFile();"><i class="fa fa-download"></i></a>
						</div>
					</div>
...
<script language="javascript" type="text/javascript">
    function uploadNewsFile() {
        ...
        $.ajax({
            ...
            error: function (result) {
				$.notify({message:'Ошибка загрузки файла!'},{type: 'error'});
            }
        });
    }

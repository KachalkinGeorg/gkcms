	...
	<tr>
		...
		<td nowrap align="center">
			{% if (entry.flags.showMain) %}
				<i class="fa fa-check-circle text-success" title="{{ lang['yesa'] }}"></i>
			{% else %}
				<i class="fa fa-times-circle text-danger" title="{{ lang['noa'] }}"></i>
			{% endif %}
		</td>
		...
		<td class="text-right" nowrap>
			{% if (flags.canModify) %}
				<div class="btn-group btn-group-sm" role="group">
					<button type="button" onclick="categoryModifyRequest('up', {{ entry.id }});" title="{{ lang['category.up'] }}" class="btn btn-outline-primary"><i class="fa fa-arrow-up"></i></button>
					<button type="button" onclick="categoryModifyRequest('down', {{ entry.id }});" title="{{ lang['category.down'] }}" class="btn btn-outline-primary"><i class="fa fa-arrow-down"></i></button>
				</div>
			{% endif %}
